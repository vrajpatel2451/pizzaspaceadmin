...
const ProductReviewsScreen = () => {
  // Query state
  const [query, setQuery] = useState<OrderItemReviewQueryParams>({
    limit: 10,
    currentPage: 1,
  });
  ...
  // Search with debounce (search on itemId)
  ...
  // Sync debounced search to query (search on itemId)
  useEffect(() => {
    setQuery((prev) => ({
      ...prev,
      itemId: debounceVal || undefined,
      currentPage: 1,
    }));
  }, [debounceVal]);
  ...
  useEffect(() => {
    setQuery((prev) => ({
      ...
      ratings: selectedRating ? parseInt(selectedRating) : undefined,
      currentPage: 1,
    }));
  }, [selectedStoreId, selectedCustomerId, selectedRating]);

  // Fetch item reviews
  const { data, isFetching } = useFetchOrderItemReviews(query);
